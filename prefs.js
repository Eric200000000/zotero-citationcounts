/* Citation Counts - preferences pane script.
 * Runs in the Zotero preferences window. `document` is the prefs document;
 * `Zotero` is available. Talks to the plugin via the Zotero.CitationCounts API
 * exposed by lib/open-citations.js.
 */
var ZOC_Prefs = {
  BRANCH: "citationcounts.",

  api() { return Zotero.CitationCounts || {}; },
  ...
  init() {
    ...
    try {
      ...
      Zotero.debug("[CitationCounts/prefs] init error: " + e);
    }
  },
