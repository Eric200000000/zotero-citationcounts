/* Zotero Citation Counts - core logic
 *
 ...
 * sidecar JSON (zotero-citationcounts-state.json in the Zotero data dir),
...
ZCC = (function () {
  ...
  // ---- config ------------------------------------------------------------
  const PREF_BRANCH = "citationcounts.";
  const DEFAULTS = {
    primarySource: "openalex",     // "openalex" | "crossref" | "semanticscholar"
    useFallback: true,             // try the other sources if the primary misses
    email: "",                     // OpenAlex/Crossref polite-pool contact (recommended)
    s2ApiKey: "",                  // optional Semantic Scholar key; S2 is OFF without it
  ...
  const PLUGIN_ID = "citationcounts@zotero.eric";
  const COLUMN_KEY = "ccCitation";
  ...
  let _columnKey = null;
  let _rootURI = "";
  let _rlSources = new Set(); // sources that 429'd this run (skipped for the rest)
  const _liveTimers = new Set();

  // ---- small helpers -----------------------------------------------------
  function log(msg) { Zotero.debug("[CitationCounts] " + msg); }
  ...
  function notify(msg) {
    ...
    try {
      ...
      pw.changeHeadline("Citation Counts");
  ...
  async function fetchSemanticScholar(item) {
    ...
    const data = await httpJson(url, key ? { "x-api-key": key } : null);
    if (!data) return null;
  ...
  // Crossref: no key, no captcha. DOI-only (its title search is too loose to
  // trust for exact matching). Count = "is-referenced-by-count".
  async function fetchCrossref(item) {
    const doi = getDOI(item);
    if (!doi) return null;
    let url = "https://api.crossref.org/works/" + doi.replace(/#/g, "%23");
    const mailto = (getPref("email") || "").trim();
    if (mailto) url += "?mailto=" + encodeURIComponent(mailto);
    const data = await httpJson(url);
    ...
    return { count: c, source: "Crossref" };
  }

  // Available sources, in fallback order. Semantic Scholar is included ONLY when
  // an API key is set (its anonymous pool 429s constantly); OpenAlex and
  // Crossref need no key, so the plugin works for everyone with zero setup.
  function sourceOrder() {
    const hasKey = !!(getPref("s2ApiKey") || "").trim();
    const available = ["openalex", "crossref"];
    if (hasKey) available.push("semanticscholar");
    const primary = getPref("primarySource");
    const order = available.includes(primary) ? [primary] : [];
    for (const s of available) if (!order.includes(s)) order.push(s);
    return getPref("useFallback") ? order : order.slice(0, 1);
  }

  function fetchBySource(src, item) {
    if (src === "openalex") return fetchOpenAlex(item);
    if (src === "crossref") return fetchCrossref(item);
    if (src === "semanticscholar") return fetchSemanticScholar(item);
  ...
  async function updateOne(item) {
    let sawRateLimit = false, skipped = false;
    for (const src of sourceOrder()) {
      // once a source 429s this run, stop hammering it for the rest of the run
      if (_rlSources.has(src)) { skipped = true; continue; }
      try {
        const res = await fetchBySource(src, item);
        ...
        if (isRateLimit(e)) { sawRateLimit = true; _rlSources.add(src); }
        else log("source " + src + " error for " + item.key + ": " + e);
      }
    }
  ...
  // ---- sidecar state -----------------------------------------------------
  const STATE_FILE = "zotero-citationcounts-state.json";
  const LEGACY_STATE_FILE = "zcc-state.json"; // pre-rename name; migrated once
  function statePath() {
  ...
  async function updateEntireLibrary() {
    ...
    if (win && items.length > 100) {
      ...
      const ok = win.confirm(
        "Citation Counts will update all " + items.length + " items in your " +
  ...
  function addToWindow(window) {
    ...
    mk("zoc-item-update", "Update citation count (Citation Counts)",
      updateSelected, "zotero-itemmenu");
    mk("zoc-collection-update", "Update citations in collection (Citation Counts)",
      updateCollection, "zotero-collectionmenu");
    mk("zoc-tools-updateall", "Update entire library now (Citation Counts)",
      updateEntireLibrary, "menu_ToolsPopup");
    mk("zoc-tools-runall", "Update stale citations now (Citation Counts)",
  ...
  function shutdown() {
    ...
    try { delete Zotero.CitationCounts; } catch (e) {}
    saveStateNow();
  }
