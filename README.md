...
A **Zotero 7, 8, and 9** plugin that adds a sortable **Citation** column
to your library, filled from open citation APIs (OpenAlex, Crossref, and
Semantic Scholar). It can keep your whole library refreshed automatically
once a day, with no clicking and no captchas.

> This is a fork of
> [josesiqueira/zotero-open-citations](https://github.com/josesiqueira/zotero-open-citations),
> which is itself an independent rewrite for the Zotero 7+ bootstrap
> architecture of the original
> [beloglazov/zotero-scholar-citations](https://github.com/beloglazov/zotero-scholar-citations)
> and the maintained [MaxKuehn fork](https://github.com/MaxKuehn/zotero-scholar-citations).
> It keeps their citation-count idea and the `ZSCC:` `extra` marker, but swaps
> Google Scholar scraping for open APIs. Full attribution in [CREDITS.md](CREDITS.md).
...
`zotero-citationcounts-state.json`, in your Zotero data directory (this keeps
...
- **Right-click an item** (or a multi-selection) -> "Update citation count (Citation Counts)". Tip: press Ctrl+A / Cmd+A in the item list first to update everything you can see.
- **Right-click a collection** -> "Update citations in collection (Citation Counts)".
- **Tools menu** -> "Update entire library now (Citation Counts)" sweeps the whole library, uncapped (it asks for confirmation past 100 items).
- **Tools menu** -> "Update stale citations now (Citation Counts)" refreshes only stale items, up to `dailyMax` per click (the same job the daily timer runs).
...
Settings -> **Citation Counts** opens a dedicated pane with all the controls
...
The same prefs are also under `extensions.zotero.citationcounts.*` in the Config
Editor:
...
zip -r -X ../zotero-citationcounts.xpi manifest.json bootstrap.js lib icons README.md CREDITS.md LICENSE
```
...
- `manifest.json` — Zotero plugin manifest (id `citationcounts@zotero.eric`).
